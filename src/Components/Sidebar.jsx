...
const Sidebar = ({ sidebar, setSidebar}) => {
  ...
  return (
    <div className = {`w-60 z-10 bg-white border-r border-gray-200 flex flex-col
    justify-between items-center max-sm:absolute top-14 bottom-0 ${sidebar? 'translate-x-0'
    : 'max-sm:-translate-x-full'} transition-all duration-300 ease-in-out text-gray-700`} >
        <div className='my-7 w-full'>
            <img src={user.imageUrl} alt="user avatar" className='w-13 rounded-full mx-auto'/>
            <h1 className='mt-3 text-center'>{user.fullName}</h1>
            <div className='px-4 mt-5 text-sm text-gray-700 font-medium'>
                {navItems.map((item, i) => (
                    <NavLink key={item.to} to={item.to} end={item.to === "/ai"} onClick={() => setSidebar(false)} className={({isActive}) => `px-3.5 py-2.5
                    flex items-center gap-3 rounded ${isActive? 'bg-linear-to-r from-[#3CB1F6] to-[#9234EA] text-white' : ''}`}>
                        <item.Icon />
                        <p>{item.label}</p>
                    </NavLink>
                ))}
