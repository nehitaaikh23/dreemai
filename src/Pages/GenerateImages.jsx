...
const GenerateImages = () => {
  ...
  const imageStyle = [
    'Realistic', 'Anime', 'Illustration', 'Fantasy',
    '3D', 'Portrait', 'Graphic Design 3D', 
  ]
  ...
  return (
    ...
    text-slate-700'>
      ...
      border-gray-200'>
        ...
        <textarea  rows={4} onChange={(e) => {setInput(e.target.value)}} value={input} className='w-full p-2 px-3 mt-2 outline-none text-sm resize-none
        rounded-md border border-gray-300' placeholder='Image Description...' required/>

        <p className='mt-4 text-sm font-medium'>Art Style</p>

        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          {imageStyle.map((item, i) => (
            <span key={i} className={`text-xs px-4 py-1 border rounded-full 
            cursor-pointer ${selectedStyle === item ? 'bg-green-50 text-green-700' : 'text-gray-500 border-gray-300'}`} onClick={() => setSelectedStyle(item)}>{item}</span>
          ))}
        </div>
